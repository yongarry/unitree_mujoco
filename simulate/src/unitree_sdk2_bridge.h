...
using OdomState_t = unitree::robot::go2::publisher::SportModeState;
...
public:
    RobotBridge(mjModel *model, mjData *data) : UnitreeSDK2BridgeBase(model, data)
    {
        ...
        odomstate = std::make_unique<OdomState_t>("rt/odommodestate");
    ...
    virtual void run()
    {
        ...
        // odom (world-frame base position + linear velocity from MuJoCo sensors)
        if(odomstate->trylock()) {
            if(frame_pos_adr_ >= 0) {
                odomstate->msg_.position()[0] = mj_data_->sensordata[frame_pos_adr_ + 0];
                odomstate->msg_.position()[1] = mj_data_->sensordata[frame_pos_adr_ + 1];
                odomstate->msg_.position()[2] = mj_data_->sensordata[frame_pos_adr_ + 2];
            }
            if(frame_vel_adr_ >= 0) {
                odomstate->msg_.velocity()[0] = mj_data_->sensordata[frame_vel_adr_ + 0];
                odomstate->msg_.velocity()[1] = mj_data_->sensordata[frame_vel_adr_ + 1];
                odomstate->msg_.velocity()[2] = mj_data_->sensordata[frame_vel_adr_ + 2];
            }
            odomstate->unlockAndPublish();
        }
        // wireless_controller
    ...
    std::unique_ptr<OdomState_t> odomstate;
