...
namespace
{
  ...
  // Apply the scene keyframe named "init" if present (see scene XML <keyframe>).
  void ApplyInitKeyframe(mjModel *model, mjData *data, mj::Simulate *sim = nullptr)
  {
    if (!model || !data)
    {
      return;
...
// previous (MuJoCo built-in) key callback, kept so we can chain to it
...
void user_key_cb(GLFWwindow* window, int key, int scancode, int act, int mods) {
  if (act==GLFW_PRESS)
  {
    ...
    if(key==GLFW_KEY_BACKSPACE) {
      ApplyInitKeyframe(m, d);
    }
  }

  // forward the event to MuJoCo's built-in handler so the default
  // shortcuts (Space, arrows, [ ], Tab, F1-F5, ...) keep working
  if (prev_key_cb) {
...
int main(int argc, char **argv)
{
  ...
  sim->run = 0;  // start paused; press Space to run
  ...
  prev_key_cb = glfwSetKeyCallback(static_cast<mj::GlfwAdapter*>(sim->platform_ui.get())->window_,user_key_cb);
  sim->RenderLoop();
