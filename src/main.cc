...
namespace
{
  ...
  std::filesystem::path getPackageShareDir(const std::string &package_name = "unitree_mujoco")
  {
    try
    {
    ...
    catch (const std::exception &)
    {
      ...
      if (std::filesystem::exists(executable_dir / "config.yaml"))
      {
        return executable_dir;
      }
      if (std::filesystem::exists(parent_dir / "config.yaml"))
      {
        return parent_dir;
      }
  ...
  void scanPluginLibraries()
  {
    ...
    // Prefer ROS package resources, but keep the original executable-relative
    // path for direct CMake runs.
    ...
    const std::filesystem::path share_plugin_dir = getPackageShareDir() / MUJOCO_PLUGIN_DIR;
    const std::string plugin_dir = std::filesystem::exists(share_plugin_dir)
                                       ? share_plugin_dir.string()
...
int main(int argc, char **argv)
{
  ...
  // Load simulation configuration from compiled defaults and CLI/launch args.
  const std::filesystem::path package_share_dir = getPackageShareDir();
  param::helper(argc, argv);
  if(param::config.robot_scene.is_relative()) {
    const std::filesystem::path description_model =
      getPackageShareDir("g1_description") / "xml" / param::config.robot_scene;
    if (std::filesystem::exists(description_model)) {
      param::config.robot_scene = description_model;
    } else {
      param::config.robot_scene = package_share_dir / param::config.robot_scene;
    }
  }
